...
#define SEM_NAME1 "file_req3"
#define SEM_NAME2 "wake_up_parent3"
#define SEM_NAME3 "answer3"
...
void* threadWorker(void* args){
    ...
    char filename[2048];
    strcpy(filename,args_->name_of_file);
    fp = fopen(filename, "r");
    if (fp == NULL)
        exit(EXIT_FAILURE);
    int i = 0;
...
int main(int argc,char** argv){
    ...
    if (pid[i] == 0){
	    ...
	    //printf("Shared temp segment with id %d attached at %p\n", shmid, shared_memory);
        shared_t = (struct shared_temp *)shared_temp;
        for(int i = 0;i < numOfFilesWanted;i++){
            struct dirent *de;
        ...
        printf("loop%d\n",numberOfProcesses*numOfFilesWanted);
