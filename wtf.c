...
#define SEM_NAME1 "file_req6"
#define SEM_NAME2 "wake_up_parent6"
#define SEM_NAME3 "answer6"
...
int main(int argc,char** argv){
    ...
    for(i = 0; i < numberOfProcesses; i++){
        ...
        printf("NAME OF SEM%s",sem_name);
        semaphore_transactions[numberOfProcesses] = sem_open(sem_name, O_CREAT | O_EXCL, SEM_PERMS, INITIAL_VALUE_WAKE);
    }
    for(i = 0; i < numberOfProcesses; i++){
        char sem_name[1024];
        sprintf(sem_name, "TS_ANS%d", i);
        semaphore_transactions_ans[numberOfProcesses] = sem_open(sem_name, O_CREAT | O_EXCL, SEM_PERMS, INITIAL_VALUE_WAKE);
    }
    for(i = 0; i < numberOfProcesses; i++){
        //printf("created!\n");
        pid[i] = fork();
